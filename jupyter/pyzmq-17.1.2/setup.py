...
# I can't get info from config file until I have a config file. So override here
ios_cross_compiling = True
...
def _add_rpath(settings, path):
    ...
    if sys.platform == 'darwin':
        # iOS 1:
        settings['extra_link_args'].extend(['-arch','arm64','-miphoneos-version-min=12.0','-isysroot','/Applications/Xcode.app/Contents/Developer/Platforms/iPhoneOS.platform/Developer/SDKs/iPhoneOS.sdk/'])
...
def settings_from_prefix(prefix=None, bundle_libzmq_dylib=False):
    ...
    if sys.platform.startswith('win'):
        ...
        # iOS 2:
        if sys.platform == 'darwin':
            settings['extra_link_args'] += ['-L',pjoin(prefix, 'lib')]
...
if cython and not ios_cross_compiling:
...
for submod, packages in submodules.items():
    if (submod == 'backend.cython' and ios_cross_compiling): 
        continue
...
if pypy or ios_cross_compiling:
