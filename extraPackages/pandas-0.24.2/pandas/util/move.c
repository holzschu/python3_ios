...
static void cython_freefunc(PyObject *m) {
    fprintf(stderr, "move module, entering free function\n");	
    // iOS: reset all global variables to default state
    // (none) and reset type:
    stolenbuf_type.tp_dict = 0;
    PyType_Reset(&stolenbuf_type);
	stolenbuf_type.tp_flags = Py_TPFLAGS_DEFAULT | Py_TPFLAGS_HAVE_NEWBUFFER | Py_TPFLAGS_HAVE_GETCHARBUFFER; 
}
